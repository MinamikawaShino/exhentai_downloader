...
[English](README.md) | [简体中文](README.zh-cn.md) | [日本語](README.jp.md) | [Русский](README.ru.md)
...
## 去重邏輯說明
- **同資料夾忽略**：同一個子資料夾內的圖片，哪怕雜湊值一樣也不應該刪除。
- **跨資料夾對比**：只有和其他資料夾交集對比之後才能保留最新、刪除舊的圖像副本。
- **資料夾去重**：當兩個資料夾內相同的圖像比例超過設定閾值（預設 50%）時，程式會刪除修改日期最早的資料夾。
- **廣告圖刪除功能**：去重時只提取資料夾中最後的 6 張圖像（按名稱排序）進行對比，如果重疊率超過設定閾值（預設 50%），則刪除修改日期最早的資料夾。
- **自訂閾值**：增加了一個設定項，讓使用者自己選擇這個重疊率比例該怎麼操作。
